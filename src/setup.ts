...
const GLOBAL_SKILL_DIR = join(homedir(), '.claude', 'skills');
...
function writeHookConfig(): void {
  ...
  // Add hooks for all scenarios that require user attention:
  // - Stop: Claude finished responding (already implemented)
  // - Notification: system notifications
  // - StopFailure: API error occurred
  // - PostToolUseFailure: tool execution failed
  // - PermissionRequest: permission dialog needs user action
  // - Elicitation: MCP is asking user for input
  const hookEvents = ['Stop', 'Notification', 'StopFailure', 'PostToolUseFailure', 'PermissionRequest', 'Elicitation'];
...
async function setupCli(): Promise<void> {
  ...
  try {
    ...
    console.error(`\n❌ 登录失败: ${e.message}`);
    console.error('   请重新运行 /wechat-cli 重试');
    process.exit(1);
  }
  writeHookConfig();
...
`;
  ...
  // Write a .bat wrapper that explicitly calls node.exe with the launcher.
  // This avoids "How do you want to open this .js file?" on systems where
  // .js file association is set to an editor instead of Node.js.
  // .bat files are always executed by cmd.exe, never opened by file association.
...
`;
  ...
  // Open new CMD window via the .bat file.
  // `start cmd /K batPath` is safe because .bat files are always run by cmd.
...
async function installSkill(): Promise<void> {
  ...
  // 2. Write wechat-cli skill
  const wechatCliContent = `---
name: wechat-cli
description: WeChat CLI mode (bidirectional). Use when user runs /wechat-cli to enable two-way WeChat communication.
---

# WeChat CLI Mode

When the user runs \`/wechat-cli\`, run this command immediately:

\`\`\`bash
wechat-claude-skill cli
\`\`\`

After the command finishes, simply repeat the command output to the user. Do NOT summarize, do NOT rephrase, do NOT add any explanation.

All messages to the user must be in Chinese.

## Unbind

When the user runs \`/unwechat\`, run \`wechat-claude-skill unbind\`.
`;

  const wechatCliDir = join(GLOBAL_SKILL_DIR, 'wechat-cli');
  mkdirSync(wechatCliDir, { recursive: true });
  writeFileSync(join(wechatCliDir, 'SKILL.md'), wechatCliContent);
  console.log(`Skill installed to: ${join(wechatCliDir, 'SKILL.md')}`);

  // 4. Write wechat-vscode skill
  const wechatVscodeContent = `---
name: wechat-vscode
description: WeChat VSCode mode (one-way notify). Use when user runs /wechat-vscode to enable WeChat notifications.
---

# WeChat VSCode Mode

When the user runs \`/wechat-vscode\`, run this command immediately:
...
`;
  ...
  const wechatVscodeDir = join(GLOBAL_SKILL_DIR, 'wechat-vscode');
  mkdirSync(wechatVscodeDir, { recursive: true });
  writeFileSync(join(wechatVscodeDir, 'SKILL.md'), wechatVscodeContent);
  console.log(`Skill installed to: ${join(wechatVscodeDir, 'SKILL.md')}`);

  // 5. Write unwechat skill
...
Tell the user WeChat has been unbound (skill still installed, use /wechat-cli or /wechat-vscode to re-bind).
`;

  const unwechatDir = join(GLOBAL_SKILL_DIR, 'unwechat');
  ...
  console.log('   /wechat-cli   — CLI 终端模式（双向通信）');
  console.log('   /wechat-vscode — VSCode 模式（单向通知）');
  console.log('   /unwechat      — 解绑微信');
}
...
function uninstallSkill(): void {
  ...
  // 2. Remove skill directories (wechat-cli, wechat-vscode, unwechat)
  const dirsToRemove = ['wechat-cli', 'wechat-vscode', 'unwechat'];
  for (const dir of dirsToRemove) {
    const skillDir = join(GLOBAL_SKILL_DIR, dir);
    try { unlinkSync(join(skillDir, 'SKILL.md')); } catch {}
    try { rmdirSync(skillDir); } catch {}
  }

  // 3. Delete account.json
  const accountPath = join(BRIDGE_DIR, 'account.json');
  if (existsSync(accountPath)) {
    unlinkSync(accountPath);
    console.log('Account data deleted');
