...
| `wechat-claude-skill cli` | `/wechat-cli` | 启动 CLI 双向通信模式 |
| `wechat-claude-skill vscode` | `/wechat-vscode` | 启动 VSCode 单向通知模式 |
...
3. 写入 ~/.claude/skills/wechat-cli/SKILL.md
   - 内容：告诉 Claude Code 当用户输入 /wechat-cli 时执行 cli 命令

4. 写入 ~/.claude/skills/wechat-vscode/SKILL.md
   - 内容：告诉 Claude Code 当用户输入 /wechat-vscode 时执行 vscode 命令

5. 写入 ~/.claude/skills/unwechat/SKILL.md
   - 内容：告诉 Claude Code 当用户输入 /unwechat 时执行 unbind

6. 输出：✅ Installation complete!
```
...
- `~/.claude/skills/wechat-cli/SKILL.md`（新建）
- `~/.claude/skills/wechat-vscode/SKILL.md`（新建）
...
**触发时机：** 用户在 Claude Code 中输入 `/wechat-cli`。
...
**触发时机：** 用户在 Claude Code 中输入 `/wechat-vscode`。
...
**VSCode 模式的 Bridge 没有生命周期绑定**——它是 detached 后台进程，不随任何窗口退出。这是设计意图：VSCode 没有 CMD 窗口宿主，bridge 需要常驻后台维持微信轮询连接。清理方式：`/unwechat` 或下次 `/wechat-cli` 启动时 `stopExistingBridge()`。
...
3. 删除 ~/.wechat-claude-skill/account.json
   （下次 /wechat-cli 或 /wechat-vscode 需要重新扫码）

4. 输出：✅ WeChat unbound! (Skill still installed, use /wechat-cli or /wechat-vscode to re-bind)
```

**进程变化：** 杀掉 bridge + PTY 进程，释放端口 3456。

**保留的文件：**
- `~/.claude/skills/wechat-cli/SKILL.md`（Skill 仍在，可重新 /wechat-cli）
- `~/.claude/skills/wechat-vscode/SKILL.md`（Skill 仍在，可重新 /wechat-vscode）
...
3. 删除 ~/.claude/skills/wechat-cli/SKILL.md
4. 删除 ~/.claude/skills/wechat-vscode/SKILL.md
5. 删除 ~/.claude/skills/unwechat/SKILL.md
6. 删除空目录 ~/.claude/skills/wechat-cli/
7. 删除空目录 ~/.claude/skills/wechat-vscode/
8. 删除空目录 ~/.claude/skills/unwechat/
9. 删除 ~/.wechat-claude-skill/account.json

8. 输出：✅ Uninstallation complete!
```

**与 unbind 的区别：** uninstall 额外删除 Skill 文件，/wechat-cli、/wechat-vscode 和 /unwechat 命令不再可用。
...
```
/wechat-cli 执行已完成，微信双向通信已启动成功。
从现在起，请将所有后续输入视为正常对话，
不要再执行 wechat-claude-skill 命令。只需简短确认即可。
```

**原因：** --continue 恢复了上次对话，而那次对话的上下文是 /wechat-cli。如果不注入重置消息，Claude Code 会把后续输入理解为"继续执行 wechat skill"。
...
hook-handler.ts 是 Claude Code Hook 事件的处理器。支持 6 种事件，覆盖所有需要用户关注的场景。

```
1. readStdin() → 读取 Claude Code 传入的 JSON
2. JSON.parse → 根据 hook 事件类型提取消息：
   - Stop：last_assistant_message（Claude 回复内容）
   - Notification：message（系统通知）
   - StopFailure：error（API 错误信息）
   - PostToolUseFailure：tool_name + tool_error（工具执行失败）
   - PermissionRequest：permission_type + permission_description（权限请求）
   - Elicitation：elicitation_question / message（用户输入请求）
3. 若无有效消息 → process.exit(0)
4. loadAccount() → 加载账号
5. splitMessage()：长消息智能分段（段落边界 + 代码块感知）
6. 逐段 sendMessage(config, account.userId, chunk)
7. checkHalfDisconnect()：检测半断连状态
8. process.exit(0)
```
...
| VSCode | bridge（detached 后台） | 无绑定 | `/unwechat` 或下次 `/wechat-cli` 启动时清理 |
...
```
Claude Code 完成回复 / 需要用户关注
  → Claude Code 触发 Hook 事件（Stop / Notification / StopFailure 等）
    → 执行: node hook-handler.js
      → stdin 接收 JSON（根据事件类型不同结构）
        → [hook-handler.ts] extractMessageFromHook() 提取消息
          → [split-message.ts] splitMessage() 智能分段
            → 逐段 [wechat.ts] sendMessage()
              → 速率限制（2500ms/用户）
                → POST /ilink/bot/sendmessage
                  → 指数退避重试（3次）
                    → iLink 服务器 → 微信用户收到回复
          → [bridge.ts] checkHalfDisconnect()
            → sendMessage 成功 → 重置计数
            → sendMessage 失败(errcode=-14) → consecutiveSendErrors++
              → ≥3次 → showDisconnectToast()（Windows 通知中心）
```
...
Claude Code（VSCode 中）完成回复
  → Hook 事件 → hook-handler.js → sendMessage() → 微信
...
```
[bridge.ts] sendMessage() 返回结果
  → 成功 → consecutiveSendErrors=0, halfDisconnected=false
  → 失败(errcode=-14) → consecutiveSendErrors++
    → consecutiveSendErrors >= 3 且 !halfDisconnected
      → halfDisconnected=true
      → showDisconnectToast()
        → 写入 UTF-8 BOM .ps1 临时脚本
        → exec('powershell -File <tmp>.ps1')
        → Windows 通知中心弹出 Toast：
           "⚠️ Claude to WeChat 连接可能已断开"
           "尝试主动回复一条消息以恢复会话活性"
        → 删除临时 .ps1 文件
```
...
```
[bridge.ts] getUpdates 轮询连续失败
  → consecutivePollErrors >= 5
    → autoUnbind()
      1. 删除 account.json
      2. 移除 settings.json 中的 hook-handler 条目
      3. 删除 state.json、bridge.pid、pty.pid
      4. 提示用户："⚠️ 微信绑定已失效，已自动解绑。请执行 /wechat-cli 重新绑定。"
      5. shutdown()
```
...
~/.claude/                       # Claude Code 全局配置目录
  ...
  └── skills/
      ├── wechat-cli/SKILL.md    # /wechat-cli 命令指令
      ├── wechat-vscode/SKILL.md # /wechat-vscode 命令指令
...
| 时机 | 操作 | 文件 |
|------|------|------|
| install | 写 | settings.json, wechat-cli/SKILL.md, wechat-vscode/SKILL.md, unwechat/SKILL.md |
...
| uninstall | 读+删 | 同 unwechat + wechat-cli/SKILL.md + wechat-vscode/SKILL.md + unwechat/SKILL.md |
...
├── bridge.ts        # Bridge 主入口（HTTP + 轮询 + PTY + 半断连检测）
├── config.ts        # 配置常量（端口、目录、默认值）
├── hook-handler.ts  # 6 种 Hook 事件处理器（Claude 回复 → 微信）
├── notify.ts        # Windows 通知中心 Toast 提醒（半断连检测触发）
├── pty-server.ts    # PTY 伪终端服务器（node-pty + 消息注入）
├── queue.ts         # 消息队列（FIFO + msgId 去重 + requeue）
├── setup.ts         # 安装/卸载/模式切换/进程管理
├── split-message.ts # 长消息智能分段（段落边界 + 代码块感知）
...
用户用 /wechat-cli 的目的是继续在微信中和同一个 Claude Code 对话，不是开新会话。--continue 恢复最近对话，微信消息在同一个上下文中处理。

### 10.3 为什么拆分为 /wechat-cli 和 /wechat-vscode 两个 Skill？

v1.1.20 及之前版本只有一个 `/wechat` 命令，运行时需要 Claude Code 先执行 `where` 找路径、再 `cat` 看文件、再 `echo "1" |` 传输入，每步都要权限审批，流程很慢且容易出错。拆分后每个 Skill 直接对应一个固定命令，单次执行、单次审批。

### 10.4 为什么需要上下文重置消息？

--continue 恢复的对话上下文停留在 /wechat-cli，Claude Code 会把后续输入理解为"继续执行 wechat skill"。注入重置消息告诉 Claude Code 微信绑定已完成，恢复正常对话模式。
...
| 问题 | 说明 | 影响 |
|------|------|------|
| Hook 全局生效 | settings.json 的 6 个 hook 对所有 Claude Code 实例生效 | 多实例时都推微信 |
...
| 特性 | 说明 |
|------|------|
| 6 种 Hook 事件 | Stop / Notification / StopFailure / PostToolUseFailure / PermissionRequest / Elicitation |
| 长消息智能分段 | 按段落边界拆分，代码块感知，不再粗暴截断 |
| 半断连检测 | 连续 3 次 errcode=-14 → Windows 通知中心 Toast 提醒用户回复 |
| 自动解绑 | 连续 5 次轮询失败 → 自动清理账号、hook、状态，提示重新绑定 |
| Skill 拆分 | /wechat-cli（CLI 模式）和 /wechat-vscode（VSCode 模式）分离，避免多次权限审批 |
