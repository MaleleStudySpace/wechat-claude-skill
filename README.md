...
# 3. 在 Claude Code 里输入
/wechat-cli        # CLI 终端模式（双向通信）
# 或
/wechat-vscode     # VSCode 模式（单向通知）
```

扫码绑定微信，然后在微信里给 Bot 发条消息——搞定。
...
1. 在 VSCode 的 Claude Code 中输入 `/wechat-vscode`
2. 扫码绑定，然后在微信中给 Bot 发一条消息激活
3. 之后 Claude 的回复自动出现在微信里
...
> ⚠️ 解绑只删除账号和 Hook 配置，Skill 文件保留。如需完全卸载，运行 `wechat-claude-skill uninstall`（或在终端手动执行）。
...
├── config.ts        # 配置常量
├── hook-handler.ts  # Claude 回复 → 推送微信（6 种 Hook 事件）
├── notify.ts        # Windows 通知中心 Toast 提醒
├── pty-server.ts    # 伪终端服务器（node-pty + Claude Code）
├── queue.ts         # 消息队列（去重 + FIFO）
├── setup.ts         # 安装/卸载/模式切换
├── split-message.ts # 长消息智能分段（代码块感知）
...
| 特性 | 说明 |
|------|------|
| 双向通信（CLI 模式） | 微信发消息 → Claude 回复，Claude 回复 → 推送微信 |
| 单向通知（VSCode 模式） | Claude 回复自动推送到微信 |
| 6 种 Hook 事件 | Stop / Notification / StopFailure / PostToolUseFailure / PermissionRequest / Elicitation |
| 长消息智能分段 | 按段落边界拆分，代码块感知，不再粗暴截断 |
| 半断连检测 | 连续 3 次 errcode=-14 → Windows 通知中心 Toast 提醒 |
| 自动解绑 | 连续 5 次轮询失败 → 自动清理账号和 Hook |
| QR 码自动打开 | 扫码时自动在浏览器弹出二维码图片 |
